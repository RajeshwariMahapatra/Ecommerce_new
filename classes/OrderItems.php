...
class OrderItems
{
    ...
    public function delete()
    {
        ...
        try {
            ...
            $st = $conn->prepare("DELETE FROM order_items WHERE order_item_id = :order_item_id LIMIT 1");
            $st->bindValue(":order_item_id", $this->order_item_id, PDO::PARAM_INT);
            $st->execute();
        } catch (PDOException $e) {
            throw new Exception("Database error: " . $e->getMessage());
    ...
    /**
     * Fetches order details from checkout page cookies and stores them in the order_items table.
     *
